...
var WebsiteService = (function () {
    ...
    WebsiteService.prototype.getWebsites = function () {
        ...
        //instead map it 
        return this.http.get("http://localhost:65058/api/websites").map(function (response) {
            return response.json();
        }).catch(this.handleError);
    };
    WebsiteService.prototype.addWebsite = function () {
        var headers = new http_1.Headers({ 'Content-Type': 'application/json' });
        var options = new http_1.RequestOptions({ headers: headers });
        var website = {
            id: 1,
            name: 'Gordon',
            description: 'Gordon is a blog publishing service that allows multi-user blogs with time-stamped entries. It was developed by Prya Labs, which wa bought by Google in 2003. generally the blogs are hosted by google in a subdomain of blogger.com. Bloggs can also be hsoted in the registered custom domain of the blogger (like www.example.com) A user can have up to 100 blogs per account',
        ...
        this.http.post("http://localhost:65058/api/websites", JSON.stringify(website), options).catch(this.handleError);
    };
